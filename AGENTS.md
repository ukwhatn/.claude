...
- **接続前提のツールは接続状態を確認してから使う**: MCP サーバー・ブラウザ拡張は、この環境で有効かを確かめてから呼ぶ。接続していないものを推測で呼ばず、必要なら接続していない旨を伝えて代替手段を提示する（接続失敗は静かに起きるため、使えないことに気付かないまま手順を進めやすい）。**接続失敗で使えないときは原因を推測せず、まずパッケージの engines と実行中のランタイムバージョンを突き合わせる**（ランタイムが古いとサーバは起動直後に exit し、接続失敗としてしか見えない）
