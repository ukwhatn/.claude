...
- 大量のファイル読み・コードベース調査・ログ解析、および小規模タスクを超える非自明な実装は、コンテキスト保護とモデル階層分離のためサブエージェントへの委譲をデフォルトとし、本体（lead）はオーケストレーション・レビューに専念する（並列機構が無い環境では逐次で代替）
...
- **質問前の詳細説明（CRITICAL）**: AskUserQuestion を実行する**前に**、独立した通常メッセージ（tool 呼び出しではない地の文）で質問の背景・判断材料（調査・検証で確定した事実）・各選択肢の内容と trade-off を詳細に説明する。選択肢の label/description だけで判断させない。**AskUserQuestion の question フィールドへの詰め込みは「説明」とみなさない**（UI 上で圧縮表示され目に入らない）。「詳細は直前の説明参照」と書くなら、その説明が直前の通常メッセージとして実在していること（実例: 2026-07 <PJ> 既知挙動の質問で、説明なしに実行→拒否→存在しない question 文を「直前の説明」と参照して再拒否）
...
- **機構拡大のユーザー確認（スコープ縮小と対称のゲート）**: 新規テーブル・新規状態/enum・新規インフラ・新規運用プロセスの導入は、**レビュー指摘由来であっても**ユーザー確認必須。既存機構で代替可能な限り既存機構を使う（実例: 2026-07 <PJ> §7方針でレビューループが<NewTable>テーブル・<新規勘定>等を段階的に積み上げ、ユーザーが全て撤回）
