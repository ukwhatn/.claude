...
**外部レビュー**: 別ベンダーの LLM（cursor / codex）でレビューする（手順: `context/agent-cli-guide.md`）。発火条件は段階ごとに異なる:
...
**Claude 自身を reviewer role で spawn する自己検証は行わない**。この規定は **lead として自分の作業をレビューに出す場合にのみ適用される**。**自分がレビュアーとして呼び出された側のときは、そのレビューを自分で完遂し、別 LLM へ再委託しない**。
...
- 計画なしでの実装開始（複雑タスク時）
- **計画の外部レビュー未実施での実装開始（複雑タスク時）**: 計画書を作ったなら実装着手前に必ず別ベンダー LLM のレビューを通す。省略できるのは外部CLIが使えない環境のときだけで、その場合も計画書と完了報告の双方に省略した旨を明記する
