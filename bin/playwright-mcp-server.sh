#!/bin/bash
# playwright-mcp を standalone HTTP server として1本だけ常駐させ、自動化専用プロファイルで
# Chrome を起動する。複数の Claude Code セッションが同時にこのサーバへ HTTP で繋ぎ、
# 同一ブラウザ・同一コンテキストを共有しつつ、各セッションは自分の current tab を独立して持てる。
#
# Why not stdio: セッションごとに MCP サーバを起こすと、拡張ブリッジ方式では拡張が同時に1接続しか
# 保持しないため後から繋いだセッションが先のセッションを切断し、永続プロファイルは同時に1インスタンス
# しか掴めないため衝突する。
#
# Why not --cdp-endpoint（ユーザーが起動した普段使いのブラウザに繋ぐ方式）: 接続ごとにブラウザ側の
# 承認ダイアログが出るうえ、ドライバの既定起動フラグ（--use-mock-keychain 等）がプロファイルを壊す
# リスクを常時使うブラウザに持ち込む。サーバ自身に専用プロファイルで起動させ、経路そのものを避ける。
set -u
