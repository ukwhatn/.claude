...
- 解決したい問題は何か
- 対象アクター（エンドユーザー / 連携先 / サポート担当 / 企画・事業側 / 開発者）
- 新規 / 既存拡張 / 置換 / 共存 のどれか
- 既知バグ修正をスコープに含むか
...
#### Q4: 出力先
- メモリディレクトリパス（デフォルト確認）
- 01 / 02 以外に作る必要があるサブドキュメント（PRD、ドキュメントツール投稿用、関係者向け FAQ 等）
...
Phase: Draft chapters (parallel)
  - 各 agent に「既存フェーズのシステム要件書のサンプル」と「01 の該当章」を渡す
