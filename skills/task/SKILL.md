---
name: task
description: 実装タスクのスキルオーケストレーター。プロダクションコードの変更を伴うタスク（新規実装・修正・リファクタ・バグ修正、および差分にコード変更を含むオペレーション依頼）の開始時に常に使用し、着手前・実装開始・エラー遭遇・コミット前・PR 前の各チェックポイントで該当スキルを発火させて完遂まで進行を管理する。複数セッション規模の進行は large-task が持ち、本スキルはその配下の各セッションで使う。/task やりたいこと でも起動でき、引数なしはカタログ表示。境界: コード変更を伴わない調査・質問応答・文書のみの編集では使わない。
...
4. **登録**: 複雑タスクではチェーンを 05_log.md に記録する（Codex: plan 機構にも登録）
