...
3. **逸脱はユーザー確認を経てのみ** — 既存パターンが明らかな技術的負債になると判断した場合に限り、負債と考える根拠を添えてAskUserQuestionで確認を取った上で逸脱してよい。無断の逸脱は禁止
...
- **コメント・docstring**: グローバル規約（コメントはWhyのみ・既存形式踏襲）に従う。本スキルで重複定義しない
