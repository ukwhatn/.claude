...
- **現在地・次の一手に書く外部の状態（済／未済・所在・待ち）には時点と開いたものを添え、添えられない項目は「未確認」と書く**（形式: @context/memory-file-formats.md「外部の状態を記す欄」。読み手は書いた時点の頭の中を持たない別セッションで、記録が古びても文面は変わらない）

## 引き継ぎ側の使い方

新セッションで該当タスクのメモリディレクトリの 50_handoff.md を読む（ディレクトリ名が不明なら /findmem で検索）。引き継ぐのは経緯で、現在地・次の一手にある状態は AGENTS.md「メモリ・issue ディレクトリ」に従って対象を開いてから使う。
