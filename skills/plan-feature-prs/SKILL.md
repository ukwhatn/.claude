...
- **判断ゲート**: 機構拡大（新テーブル・新 enum・新ライブラリ・新インフラ）はユーザー承認必須。ただしユーザーが「長期的に最適な判断を支持」等で事前委任している場合は自律決定してよい — その場合は比較検討と採用理由を `99_history.md` に記録する
...
- **E2E・横断テストの扱いは計画時に決める**: 決めずに始めると実装途中で「各PRに同梱 / 専用PRでまとめ / 書かない」の判断が割れる（実例あり）。既存E2Eが壊れる分の修正は常に各PR持ち
