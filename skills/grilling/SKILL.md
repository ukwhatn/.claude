---
name: grilling
description: 既存の計画・設計・アイデアを尋問形式でストレステストし、前提と未決定事項を洗い出す対話スキル。「グリルして」「grillして」「この計画を詰めて」「計画の穴を突いて」「ストレステストして」等の依頼時、実装着手前に計画の前提を検証したい時に使用。要件書等は生成せず、確定した重要決定だけを ADR に記録する。境界: ゼロからの要件定義は design-feature、完成文書のレビューは doc-review。
---
