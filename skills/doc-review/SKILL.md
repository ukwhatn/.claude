---
name: doc-review
description: 設計書・仕様書・計画書等のドキュメントを複数エージェントで並列レビューする。/doc-review 実行時、「チームでレビューして」「多角的にレビューして」等の明示依頼時に使用。「レビューして」だけの依頼では発火しない（単独レビューで対応し、必要なら本スキルを提案する）。
---
...
6 Agent の担当観点と起動時刻を 05_log.md に1行ずつ記録する（完了は harness の通知で受け取り、同じ行に結果ファイルのパスを追記する）。
