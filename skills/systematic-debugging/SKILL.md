...
description: バグ・エラー・テスト失敗・ビルド失敗・予期しない挙動に遭遇した時、修正案を出す前に使う根本原因調査の規律（根本原因調査→パターン分析→仮説検証→実装の4フェーズ）。症状への場当たり修正を防ぐ。境界: 原因特定後の修正実装は writing-code、提出前の diff 確認は self-review。
