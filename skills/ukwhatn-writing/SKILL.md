...
description: 日本語の文章を ukwhatn 本人の文体で書く・下書きする・推敲する。PR 概要欄、レビュー指摘・返信、Slack メッセージ、チーム内ドキュメント、対外ビジネスメール、記事・レポート・議事録・提案書等の長文を日本語で書く時、本人の代わりに文章を書く時に使用。AI 特有の冗長さ・難語・翻訳調を排し、相手と媒体に応じて敬体/常体・砕け方を切り替える。境界: 実装修正を伴う PR コメント対応は pr-comment（本スキルは文章作成のみ）。
---
...
1. **STEP 1** で「相手・媒体・用途・最終的な貼り先」を確定し、文体と参照先 reference を決める。**貼り先に既存のやり取り（同じスレッド・同じ相手との直近の投稿）があれば、その形式を型にする**（媒体の一般則より、そこで実際に使われている形が優先。取得済みで手元にあるものを見ずに書き始めない）。情報が足りなければ確認する。複数媒体が絡むときは貼り先の媒体を優先する。
