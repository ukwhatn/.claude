...
- **ユーザー明示指示のスキップ禁止**: ユーザーが「X等で調査してほしい」「Y も含めて」等と項目を明示している場合、**効率理由（コンテキスト圧迫・時間節約・自分の知見で十分等）で自主的にスキップしない**。スキップ判断が必要なら AskUserQuestion で確認する（実例: 「WebSearch でOWASP等を調査してほしい」に対し「知見で十分」と自主判断→ユーザーが「調査を実施してください」で訂正）
