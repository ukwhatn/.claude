...
| 完了の通知 | **成果物を書き終えたら lead へ完了を送ってから終了する**（Claude Code の pane なら `SendMessage` で `--lead-name` の宛先へ、codex なら `herdr agent prompt`）。送る内容は成果物のパスと要点数行。これが無いと委譲先は standalone で止まり、lead は完了を検知できずに待ち続ける |
