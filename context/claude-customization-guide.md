...
### Agents（カスタムエージェント）を選ぶ場合
- 特定タスクに**ツール制限**を設けたい場合（例: 読み取り専用）
- **専門的なシステムプロンプト**で動作を特化したい場合
- `memory`で**セッション間知見蓄積**が必要な場合
- subagentとして自動委譲させたい場合

### Agent Teamsを選ぶ場合
- チームメイト間の**議論・情報共有**が必要な場合
- **並列作業**で同一モジュールのインターフェースを調整する場合
- **competing hypotheses**で複数仮説を検証する場合
- コストより**品質・速度**を優先する場合
