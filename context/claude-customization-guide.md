...
- **毎ターン・毎作業に宣言文・記録・確認の往復を挟ませる規則は、防いでいる失敗がそのコストを上回ることを確かめてから書く**。儀式は規則の数だけ積み重なり、システムプロンプトの既定動作（そのまま実行する・簡潔に書く）を打ち消す
...
- **強調（IMPORTANT / YOU MUST）は既定では付けない**。今のモデルは平叙文の指示にも従い、強調は適用しすぎを招く。付けるのは、軽視されると実際に確かめた指示1つに限り、理由を並べて書く
...
- 委譲要否は独立した作業単位の数で判定する（同上「委譲判断」参照）
