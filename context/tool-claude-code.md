...
委譲の目的は並列性だけでなく、lead（会話・計画・統合）と実装のモデル階層分離、およびコンテキスト保護（実測: 委譲セッションはトークン消費約半分・compaction 1/6）。leadは結論のみを受け取り、**報告は一次情報と突き合わせて検証してから採用する**（Writer/Reviewer分離、@context/claude-customization-guide.md §8）。
