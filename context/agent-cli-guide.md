...
差分と既存経路を横に見るだけでは、呼び出している共通処理の内部（例: 副作用の順序、部分的失敗時の状態）に起因する不具合が出ない。実測: レビュー1巡で Action Required 0 だった PR に、人間レビュアーが「共通ラッパーが DB 更新の後に監査を書くため、監査だけ失敗すると呼び出し側が古い状態を返す」型の指摘を付けた（2026-08 tokoshie）。
