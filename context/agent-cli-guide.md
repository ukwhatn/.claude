...
|-----|------|-----------|--------------|---------|
| cursor（優先） | `agent`（= `cursor-agent`） | `gpt-5.5-high-fast` | `--resume <session_id>` | `--output-format json` → `jq -r '.result'` |
| codex（fallback） | `codex exec` | `gpt-5.4`（`-c model="..."` で変更可） | `codex exec resume --last`（CWDの直近セッション） | `--json` → `jq` で `agent_message` を抽出、または `-o <file>` |
...
# 2回目以降（セッション継続）
agent -p "<プロンプト>" --resume <session_id> --trust --model gpt-5.5-high-fast --output-format json 2>/dev/null | jq -r '.result'
```
...
codex exec --model gpt-5.4 -c model_reasoning_effort="high" --json "<プロンプト>" 2>/dev/null \
...
| `--model <model>` | 使用モデル（gpt-5.5-high-fast推奨） |
...
| `--model, -m <model>` | 使用モデル（例: `gpt-5.4`, `gpt-5.3-codex`） |
| `-c model_reasoning_effort="high"` | reasoning深度（low/medium/high/xhigh）。任意 |
...
以下の例は **cursor（`agent`）** 表記。**codex環境では「基本コマンド > codex」の形に読み替える**（`agent -p "<P>" ... | jq -r '.result'` → `codex exec --model gpt-5.4 --json "<P>" | jq -r 'select(.type=="item.completed" and .item.type=="agent_message") | .item.text'`、再レビューは `codex exec resume --last --json "<P>"`）。プロンプト本文（レビュー観点・Severity分類の指示）は両CLI共通。
...
| CLI | モデル | 特徴 | 推奨用途 |
|-----|--------|------|----------|
| cursor | `gpt-5.5-high-fast` | 高精度+高速（デフォルト） | コードレビュー全般 |
| codex | `gpt-5.4` | 最新汎用フラグシップ | コードレビュー全般（fallback時） |
| codex | `gpt-5.3-codex` | コード特化 | コード変更レビュー |

```bash
# cursor: モデル一覧を確認
agent --list-models
# codex: モデルは --model / -m または -c model="..." で指定。reasoningは -c model_reasoning_effort="high"
```
