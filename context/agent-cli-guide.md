...
|-----|------|-----------|--------------|---------|
| cursor（優先） | `agent`（= `cursor-agent`） | `gpt-5.5-extra-high-fast` | `--resume <session_id>` | `--output-format json` → `jq -r '.result'` |
...
- **cursor**: effort/speedをモデル名に含む合成slug（例: `gpt-5.5-extra-high-fast`）。gpt-5.5系のxhigh相当slugは `extra-high` 表記であり、`gpt-5.5-xhigh-fast` は存在せずエラーになる（実測。gpt-5.4以前やclaude系は `xhigh` 表記のまま）。利用可能なslugは `agent --list-models` で確認（extra-high variantが無ければ `gpt-5.5-high-fast` にfallback）
- **codex**: モデルslug（`gpt-5.5`）とreasoning effort（`-c model_reasoning_effort="xhigh"`）を別指定。`gpt-5.5-extra-high-fast` のような合成名は**存在しない**。速度（Fast）はservice tierでgpt-5.5では既定
...
```bash
# 初回（session_idを取得）
agent -p "<プロンプト>" --trust --model gpt-5.5-extra-high-fast --output-format json 2>/dev/null | jq -r '.session_id, .result'

# 2回目以降（セッション継続）
agent -p "<プロンプト>" --resume <session_id> --trust --model gpt-5.5-extra-high-fast --output-format json 2>/dev/null | jq -r '.result'
```
...
| `--model <model>` | 使用モデル（gpt-5.5-extra-high-fast推奨） |
...
指摘がなければ「指摘なし」とだけ回答してください。" \
  --trust --model gpt-5.5-extra-high-fast \
...
| CLI | 指定 | 特徴 | 推奨用途 |
|-----|--------|------|----------|
| cursor | `--model gpt-5.5-extra-high-fast`（合成slug） | 最深reasoning+Fast | コードレビュー標準 |
...
```bash
# cursor: 利用可能な合成slug一覧を確認（extra-high variantが無ければ gpt-5.5-high-fast にfallback）
agent --list-models
