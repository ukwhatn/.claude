...
agent cliのnon-interactive modeを使用して、別モデル（gpt-5.4-high-fast）によるレビューを実施する。
...
# 2回目以降（セッション継続）
agent -p "<プロンプト>" --resume <session_id> --trust --model gpt-5.4-high-fast --output-format json 2>/dev/null | jq -r '.result'
```
...
| `--model <model>` | 使用モデル（gpt-5.4-high-fast推奨） |
...
| モデル | 特徴 | 推奨用途 |
|--------|------|----------|
| `gpt-5.4-high-fast` | 高精度+高速（デフォルト） | コードレビュー全般 |
