...
**本ファイルは末尾まで読んでからコマンドを組む**（抜き読みしない規定は AGENTS.md「Read when」）。コマンド形は「基本コマンド」のテンプレートをそのまま使い、レビュー種別ごとの節にあるプロンプト本文を差し込む。実行が空振りする条件は末尾「注意事項」に、ループを止める条件は「レビューループの流れ」にあり、どちらもコマンド例の中には現れない。**ラウンド上限はレビュー対象の種別で異なる**ため、対象が設計文書（計画・方針・要件ドキュメント）か実装差分かを先に確定させてから始める。
...
**レビュー本体はこちらを既定にする。** レビューはツールのタイムアウトを超えることが多く、前景で起動しても途中で背景に回る。背景に回った時点でパイプ形は出力を隠すので、最初から `-o` でファイルに落とし、ログも別ファイルへ流し、起動後は AGENTS.md「バックグラウンドプロセスの後始末」に従って進んでいる証拠を確認してからターンを終える:
...
**AR定義の境界（機構拡大の抑制）**: 新機構の導入（新規テーブル・状態/enum・インフラ・運用プロセス）を伴う指摘は、テールリスクを「データ損失」と表現していてもARとして自動修正しない。Recommended扱いとし、採否はユーザー確認に回す（AGENTS.md「中長期最適と機構追加」「ユーザーへの質問」）。**事実として正しい指摘と採用すべき指摘は別物** — 採否は費用対効果（発生確率×発生時の損失 vs 実装・保守コスト）で判断し、棄却理由は05_log.mdに記録する。
...
- Action Required = 0
- 同一指摘が2ラウンド連続
- 安全上限: 実装差分は5ラウンド、**設計文書（計画・方針・要件ドキュメント）は2〜3ラウンド**
```
...
### CRITICAL: `codex exec` は stdin を読む（ハーネスから起動すると前景でも止まる）

**`codex exec` はプロンプトを位置引数で渡しても stdin を読もうとする**（起動時に `Reading additional input from stdin...` を出す）。エージェントのツール経由で起動した Bash は stdin が端末ではなく開いたままなので、**前景・背景を問わず**そこで待ち続け、レビューが一度も走らない。前景ならツールのタイムアウトまで待ったあと背景に回り、そのまま何も起きない。

stderr を捨てている・ログに流したまま読んでいないと、この待機メッセージが目に入らず「実行したつもりで空振り」になる。

**ルール: 防ぎたいのは「codex が入力を待ったまま止まる」ことであり、ツール経由で `codex exec` を起動するときは常に stdin を閉じる（`< /dev/null` はそのための手段）。** 前景か背景かで判断しない。「基本コマンド」の全例に付いているのはこのため。
