...
**セクションを抜き読みするなら、コマンドを組むための節に加えて「レビューループの流れ」を必ず含める。** そこにラウンド上限・打ち切り条件・`/gal-check` の発動条件がある。**上限はレビュー対象の種別で異なる**ため、対象が設計文書（計画・方針・要件ドキュメント）か実装差分かを先に確定させてから始める。コマンド例と Severity 分類だけを読むと、**ループを止める条件を持たないまま回し始めることになり、超過したことは回し終えるまで分からない**。
