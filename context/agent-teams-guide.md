...
**idle通知への対応（IMPORTANT）:**
- idle通知はチームメイトの各ターン終了時に**自動的に送信される通常のイベント**
- チームメイトが複数タスクを担当している場合、タスク間でidle→再開を繰り返すのは正常動作
- **idle通知を受けても「続けてください」等の催促メッセージを送らない**（チームメイトは自律的に次タスクに進む）
- SendMessageが必要なケース: チームメイトが**明示的に質問・ブロック報告を送ってきた場合のみ**
- 進捗確認が必要なケース: **長時間（5分以上）idle状態が続き、かつTaskListでタスクが進んでいない場合のみ**

**理由:** `sleep` を使用するとプロセスがブロックされ、Teammate/Subagentからの完了報告メッセージを受信できなくなる。ターンを終了して待機状態にすることで、メッセージが即座に配信される。過剰な催促はチームメイトのコンテキストを消費し、作業効率を低下させる。
