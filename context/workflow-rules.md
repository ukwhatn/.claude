...
1. **コミットを作成する場合は（非コードのdocs等でも）**、そのリポジトリでの直コミット可否を後述「直コミット可否の判定」で確認する。チーム開発リポジトリでは作業ブランチ（feature/*等）を用い、main/develop直コミットは禁止。対象外は**コミットしない作業のみ**（メモリディレクトリ編集・読み取り専用調査）
