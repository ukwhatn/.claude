...
### ユーザーへのClaude review依頼

計画完了後、ユーザーにClaude reviewの実行を依頼する。

**依頼時に伝える内容:**
1. レビュー対象: 計画内容（30_plan.mdのパス）
2. 実行コマンド例を提示
3. 指摘があれば共有してもらうよう依頼

**依頼例:**
```
計画が完了しました。以下のコマンドで別セッションからレビューを実行してください:

claude -p "以下の計画をレビューしてください。抜け漏れ、リスク、改善点を指摘してください。
指摘がなければ「指摘なし」とだけ回答してください。

$(cat ${MEMORY_DIR}/memory/<task>/30_plan.md)" --output-format text

指摘があれば共有してください。対応後、再度レビューを依頼します。
```

**ループ:**
1. ユーザーがレビューを実行し、結果を共有
2. 「絶対にやるべき」指摘は必ず修正
3. それ以外はやる/やらない判断、またはAskUserQuestionで確認
4. 修正後、再度ユーザーにレビュー実行を依頼
5. 指摘がなくなるまで繰り返し
...
自動チェック完了後、ユーザーにClaude reviewの実行を依頼する。

**依頼時に伝える内容:**
1. レビュー対象: コード変更（git diff）
2. 実行コマンド例を提示
3. 指摘があれば共有してもらうよう依頼

**依頼例:**
```
実装が完了しました。以下のコマンドで別セッションからレビューを実行してください:
